  ...
  Widget _buildBottomButton() {
    return BlocConsumer<LoginCubit, LoginState>(
      listener: (context, state) {
        if (state is LoginSuccess && widget.isSign) {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (_) => const MainNavigationScreen()),
          );
        } else if (state is OTPPending) {
          // Navigate to OTP only once
          Navigator.push(
            context,
            MaterialPageRoute(
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                state.message,
                style: AppTypography.bodySmSemibold
                    .copyWith(color: AppColors.buttonTextPrimary),
              ),
          ...
          child: ElevatedButton(
            onPressed: _isButtonEnabled
                ? () {
                    ...
                    if (widget.isSign) {
                      // SIGN IN → call login endpoint
                      loginCubit.signInWithEmail(
                        _emailController.text,
                        _passwordController.text,
                      );
                    } else {
                      // SIGN UP → call register endpoint
                      loginCubit.registerUser(context);
                    }
                  }
                : null,
            style: ElevatedButton.styleFrom(
              backgroundColor: _isButtonEnabled && !isLoading
