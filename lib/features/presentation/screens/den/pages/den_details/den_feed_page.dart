...
class _DenFeedPageState extends State<DenFeedPage>
  ...
  initState() {
    super.initState();
    _bloc =
        DenFeedBloc(communityDenRepository, context.read<FeedManagerCubit>());
    _bloc.add(LoadFeeds(id: widget.den.id, feedType: FeedType.den));
    // fetchDens()
  }
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            Expanded(
              child: BlocBuilder<FeedManagerCubit, FeedManagerState>(
                builder: (context, state) {
                  const FeedType den = FeedType.den;
                  final List<Post> posts =
                      state.feedMap[den]?.posts ?? <Post>[];

                  if (state.isLoading(den)) {
                    return const Center(child: CircularProgressIndicator());
                  } else if (state.hasError(den).isNotEmpty) {
                    return Padding(
                      ...
                      child: Center(
                        child: Column(
                          children: [
                            ...
                            TextButton(
                                onPressed: () {
                                  context.read<DenFeedBloc>().add(RefreshFeeds(
                                      id: widget.den.id,
                                      feedType: FeedType.den));
                                },
                  ...
                  return RefreshIndicator(
                    ...
                    child: PaginatedListView<Post>(
                      ...
                      fetchMore: () async {
                        ...
                        context.read<DenFeedBloc>().add(LoadMoreFeeds(
                            id: widget.den.id, feedType: FeedType.den));
                      },
                      itemBuilder: (context, post) => FeedCard(
                        ...
                        onToggleBookMark: (post, saved) {
                          context.read<DenFeedBloc>().add(UpdateBookmark(
                              postId: post.id, isBookmarked: saved));
                          context
                              .read<FeedManagerCubit>()
                              .toggleBookmark(post.id, saved);
                        },
                      ),
                      emptyWidget: const Align(
                          alignment: Alignment.center, child: Text("No posts")),
                    ),
                  );
                },
              ),
