...
class _AppointmentCompanionScreenState extends State<AppointmentCompanionScreen> {
  ...
  Future<void> _addCustomQuestion(String sectionKey, String question) async {
    ...
    try {
      ...
      // Prepare the request data based on section type
      Map<String, dynamic> requestData = {
        'title': _title ?? 'Appointment Preparation Guide',
        'symptoms_custom_questions': [],
        'conditions_custom_questions': [],
        'history_custom_questions': [],
        'my_appointment_notes': [],
      ...
      switch (sectionKey) {
        case 'symptoms':
          requestData['symptoms_custom_questions'] = [..._symptomsList, question];
          break;
        case 'healthHistory':
          requestData['history_custom_questions'] = [..._historyList, question];
          break;
        case 'condition':
          requestData['conditions_custom_questions'] = [..._conditionsList, question];
          break;
        case 'emotionalSupport':
          requestData['my_appointment_notes'] = [_emotionalSupport ?? '', question];
          break;
      }
      ...
      // Reload companion details and AI response to get updated data
      ...
      setState(() {
        _isLoading = false;
        // Force rebuild to show updated questions
      });
  ...
  Future<void> _updateAppointmentDate(DateTime newDate) async {
    final companionId = widget.appointmentData['companionData']['companionId'] as int?;
    if (companionId == null) return;
    ...
    try {
      ...
      // Reload companion details to get updated data
      await _loadCompanionDetails();
  ...
  void _unhideQuestions() {
    Navigator.of(context).pop(); // Close the menu
    // TODO: Implement unhide questions functionality
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Unhide questions functionality coming soon!'),
        backgroundColor: Colors.blue,
      ),
    );
  }
  ...
  void _manageTags() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Manage tags functionality coming soon!'),
        backgroundColor: Colors.blue,
      ),
    );
  }
  ...
  Widget _buildEmotionalSupportSection() {
    ...
    return Column(
      children: [
        ...
        if (isExpanded) ...[
          const SizedBox(height: 16),
          if (_emotionalSupport != null && _emotionalSupport!.isNotEmpty)
            _buildEmotionalSupportContent(_emotionalSupport!)
          else
    ...
    if (_error != null) {
      return Center(
        child: Column(
          ...
          children: [
            Text(
              'Error loading appointment details: $_error',
              style: AppOSTextStyles.osMd.copyWith(color: Colors.red),
              textAlign: TextAlign.center,
            ),
            const SizedBox(height: 16),
            ElevatedButton(
              onPressed: _loadCompanionDetails,
